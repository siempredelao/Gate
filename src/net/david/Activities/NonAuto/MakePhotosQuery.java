...
public final class MakePhotosQuery extends Query {
	...
	public void insertaElementos(	QueryProperty queryProperty,
		...
		MakePhotosProperty cameraProperty = (MakePhotosProperty) queryProperty;
		...
		LayoutParams gvlp = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
		gvlp.height = LayoutParams.WRAP_CONTENT;
		gvlp.width = LayoutParams.WRAP_CONTENT;
		gvlp.addRule(RelativeLayout.ALIGN_LEFT, sfv.getId());
		gvlp.addRule(RelativeLayout.ALIGN_RIGHT, sfv.getId());
		gvlp.addRule(RelativeLayout.ALIGN_TOP, sfv.getId());
		gvlp.addRule(RelativeLayout.ALIGN_BOTTOM, sfv.getId());
		
		GridView gv = new GridView(act);
		gv.setNumColumns(3);
		gv.setLayoutParams(gvlp);
		gv.setVisibility(GridView.INVISIBLE);
		gv.setId(Constantes.GALERIA_CAMARA);
		contenedor.addView(gv);
	}
}
